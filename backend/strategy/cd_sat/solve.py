...
def solve_cp_sat(
    *,
    ...
    x = {}
    for p in pros:
        for di, d in enumerate(demand_list):
    ...
    for i in range(len(demand_list)):
        for j in range(i + 1, len(demand_list)):
            di, dj = demand_list[i], demand_list[j]
            if di["day"] != dj["day"]:
                continue
    ...
    costs = []
    if allow_unassigned:
        costs.extend(unassigned_penalty * u[di] for di in range(len(demand_list)))
    ...
    if status == cp_model.INFEASIBLE:
        ...
        diagnose_infeasibility(demand_list, pros)
    ...
    for day in range(days):
        ...
        day_items = [(gi, d) for gi, d in enumerate(demand_list) if d["day"] == day_num]
