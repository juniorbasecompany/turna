 ...
 *
 * Lista todos os memberships do tenant atual.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_URL}/membership/list${queryString ? `?${queryString}` : ''}`
    ...
    console.error('Erro ao listar memberships:', error)
    return NextResponse.json(
      {
        detail:
          error instanceof Error
            ? error.message
            : 'Erro desconhecido ao listar memberships',
      },
      { status: 500 }
    )
  }
